...
static int vesc_motor_poles_s[vesc_motor_nums]={0,7,7,7};//电机的极对数,这里从下标为1开始对应1号电机，下标为0的数据无用
...
void Vesc_speed_control_init(void);//所有vesc初始速度设为0
/**************外部接口end**************/
...
static void Write_database_vesc_extid(uint32_t id_num)
{
	...
	TxMessage.Identifier 	= 	can_database_g[hash_table[id_num]].Data_ID;
	TxMessage.IdType		=	FDCAN_EXTENDED_ID;
	TxMessage.TxFrameType = FDCAN_DATA_FRAME;
	TxMessage.DataLength = can_database_g[hash_table[id_num]].Data_length;
	...
	{
		while(HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &TxMessage, can_database_g[hash_table[id_num]].Data_ptr) != HAL_OK)
		{
			sendnum++;
			if(sendnum==10)
			{
	...
	else if(can_database_g[hash_table[id_num]].Channel==2)
	{
		while(HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &TxMessage, can_database_g[hash_table[id_num]].Data_ptr) != HAL_OK)
		{
			sendnum++;
			if(sendnum==10)
			{
	...
	else if(can_database_g[hash_table[id_num]].Channel==3)
	{
		while(HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan3, &TxMessage, can_database_g[hash_table[id_num]].Data_ptr) != HAL_OK)
		{
			sendnum++;
			if(sendnum==10)
			{
