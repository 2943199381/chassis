...
void Pid_parameter_init(void){
	...
	Pid_increment_struct_init(&motor_3508_pid_g[1].spd,  16.7f,	0.98f,	0.024314f,  15000,  -15000);

	Pid_increment_struct_init(&(motor_3508_pid_g[2].loc), 0.15, 	0.001,	0.035,  200,    -200);
	Pid_increment_struct_init(&(motor_3508_pid_g[2].spd),   16.7f,	0.98f,	0.024314f,  9500,  -9500);
	...
	Pid_increment_struct_init(&motor_3508_pid_g[6].spd,  12.0, 	1.2,	  0.035,   13000,  -13000);

	Pid_increment_struct_init(&motor_3508_pid_g[7].loc,  0.15, 	0.001,	0.03,  1000,    -1000);//2006
...
void Pid_parameter_init(void){
	...
	Pid_increment_struct_init(&motor_3508_pid_g[1].loc,   0.15, 	0.001,	0.035,   350,    -350);
	Pid_increment_struct_init(&motor_3508_pid_g[1].spd,  16.7f,	0.98f,	0.024314f,  10000,  -10000);

	Pid_increment_struct_init(&(motor_3508_pid_g[2].loc),  0.15, 	0.001,	0.035,   300,    -300);
	Pid_increment_struct_init(&(motor_3508_pid_g[2].spd),  24.0, 	1.2,	  0.03,  9500,  -9500);

	Pid_increment_struct_init(&motor_3508_pid_g[3].loc,  0.15, 	0.001,	0.035,   300,    -300);
	Pid_increment_struct_init(&motor_3508_pid_g[3].spd,  24.0, 	1.2,	  0.03,  9500,  -9500);

	Pid_increment_struct_init(&motor_3508_pid_g[4].loc,  0.15, 	0.001,	0.03,   50,    -50);
	Pid_increment_struct_init(&motor_3508_pid_g[4].spd,  16.7f,	0.98f,	0.024314f,  10000,  -10000);

	Pid_increment_struct_init(&motor_3508_pid_g[5].loc, 0.364706f,	0.007333f,	0.004745f,   100,    -100);
	Pid_increment_struct_init(&motor_3508_pid_g[5].spd,   16.7f,	0.98f,	0.024314f,  10000,  -10000);

	Pid_increment_struct_init(&motor_3508_pid_g[6].loc,  0.15, 	0.001,	0.03,  300,    -300);
	Pid_increment_struct_init(&motor_3508_pid_g[6].spd,  24.0, 	1.2,	  0.035,   9500,  -9500);

	Pid_increment_struct_init(&motor_3508_pid_g[7].loc,  0.15, 	0.001,	0.03,  450,    -450);
	Pid_increment_struct_init(&motor_3508_pid_g[7].spd,  24.0, 	1.2,	  0.035,   16000,  -16000);

    Pid_increment_struct_init(&motor_3508_pid_g[8].loc,  0.15, 	0.001,	0.03,  240,    -240);
	Pid_increment_struct_init(&motor_3508_pid_g[8].spd,  24.0, 	1.2,	  0.035,   9500,  -9500);
...
//	if(pid_struct==&motor_3508_pid_g[1].loc && Basic_float_abs(position-target)<40){
//			delta_pid_cal_result =0;
//			pid_cal_result += delta_pid_cal_result;
//	}
